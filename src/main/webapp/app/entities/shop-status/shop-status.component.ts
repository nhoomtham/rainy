...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/shop-status'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: ShopStatus) {
        return item.id;
    }
    registerChangeInShopStatuses() {
        this.eventSubscriber = this.eventManager.subscribe('shopStatusListModification', (response) => this.loadAll());
    }
